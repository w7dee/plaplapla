--[[
 * [SECURITY PATCH]
 * الملف ده كان فيه 5 دوال مسجّلة كـ command handler و remote event في نفس الوقت.
 * في الـ command الباراميتر الأول بيحطه السيرفر (اللاعب الحقيقي).
 * في الـ event الباراميتر الأول كان بيحطه الكلاينت -> كان بيبعت عنصر أدمن
 *   فيعدّي فحص الصلاحية. ده اسمه "confused deputy".
 * الحل: الأحداث بقت بتنده الدالة بـ `client` مباشرة، والباراميتر اللي بيبعته
 *   اللاعب بيتترمى.
]]
...
local function secureCmdEvent(eventName, handler, perm, cooldown)
	...
	addEventHandler(eventName, root, function(...)
		...
		-- الباراميتر الأول دايمًا اللاعب الحقيقي، مش اللي الكلاينت بعته
...
secureCmdEvent("onMakeGun", givePlayerGun, "admin", 250)
...
secureCmdEvent("admin-system:takeItem", takePlayerItem, "trialadmin", 250)
...
secureCmdEvent("remoteFreezePlayer", freezePlayer, "trialadmin", 250)
...
secureCmdEvent("admin-system:adminduty", adminDuty, "trialadmin", 1000)
...
secureCmdEvent("admin-system:gmduty", gmDuty, "supporter", 1000)
